...
// Caelum Sky Dome without Haze fragment program in GLSL
...
{
	// Pass the colour
	gl_FragColor = texture2D(gradientsMap, gl_TexCoord[0].xy + vec2(offset, 0.0)) * gl_Color;
	...
	if(gl_TexCoord[1].x > 0.0)
	{
		...
		float scatteredSunlight = pow(incidenceAngleCosClamped, log(sunlightScatteringFactor * 0.5) / log(0.5));
		float absorption = clamp(atmLightAbsorptionFactor * (1.0 - texture1D(atmRelativeDepth, gl_TexCoord[1].y).r), 0.0, 1.0);
